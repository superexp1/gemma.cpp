...
// Same as `RMSNorm` without weights, non-in-place version.
template <typename VecT, typename OutT>
HWY_NOINLINE void RMSNormNoScale(const VecT* HWY_RESTRICT x,
                                 OutT* HWY_RESTRICT out, const size_t size,
                                 ThreadingContext& ctx, const size_t worker) {
  GCPP_ZONE(ctx, worker, Zones::kOpsRmsNormNoScale);
  HWY_DASSERT(reinterpret_cast<uintptr_t>(out) !=
              reinterpret_cast<uintptr_t>(x));

  const float mul = detail::RMSNormMul(x, size, ctx, worker);
  MulByConstTo(mul, x, out, size, ctx, worker);
}
...
void RMSNormNoScaleBatched(
  ...
  ParallelFor(parallelism, in.Rows(), ctx, cluster_idx,
              ...
              [&](uint64_t token_idx, size_t worker) {
                RMSNormNoScale(in.Row(token_idx), out.Row(token_idx), in.Cols(),
                               ctx, worker);
              });
}
