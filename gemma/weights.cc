...
void LayerWeightsPtrs::Fixup(Model model,
  ...
  auto convert_clamp = [](const MatPtr& min_ptr,
                          const MatPtr& max_ptr) -> ClampRange {
    ClampRange range;
    if (min_ptr.HasPtr() && max_ptr.HasPtr()) {
      HWY_DASSERT(min_ptr.GetType() == Type::kBF16);
      HWY_DASSERT(max_ptr.GetType() == Type::kBF16);
      range.min = hwy::ConvertScalarTo<float>(
          *static_cast<const BF16*>(min_ptr.Packed()));
      range.max = hwy::ConvertScalarTo<float>(
          *static_cast<const BF16*>(max_ptr.Packed()));
    }
    return range;
  };
  qc_in = convert_clamp(qc_in_min_matptr, qc_in_max_matptr);
  qc_out = convert_clamp(qc_out_min_matptr, qc_out_max_matptr);
  kvc_in = convert_clamp(kvc_in_min_matptr, kvc_in_max_matptr);
  kvc_out = convert_clamp(kvc_out_min_matptr, kvc_out_max_matptr);
  aoc_in = convert_clamp(aoc_in_min_matptr, aoc_in_max_matptr);
  aoc_out = convert_clamp(aoc_out_min_matptr, aoc_out_max_matptr);
  gtc_in = convert_clamp(gtc_in_min_matptr, gtc_in_max_matptr);
  gtc_out = convert_clamp(gtc_out_min_matptr, gtc_out_max_matptr);
  linc_in = convert_clamp(linc_in_min_matptr, linc_in_max_matptr);
  linc_out = convert_clamp(linc_out_min_matptr, linc_out_max_matptr);
}
