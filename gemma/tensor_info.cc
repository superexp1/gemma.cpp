...
void TensorInfoRegistry::AddLayerTensors(const ModelConfig& config,
  ...
  Add(suffix,
      {
          ...
          .source_names = {"attn/kv_einsum/ClippedEinsum_0/clip_input_min",
                           "attn/k_einsum/ClippedEinsum_0/clip_input_min"},
          .axes = {0},
          .shape = {1},
          .min_size = Type::kBF16,
      });
  Add(suffix,
      {
          ...
          .source_names = {"attn/kv_einsum/ClippedEinsum_0/clip_input_max",
                           "attn/k_einsum/ClippedEinsum_0/clip_input_max"},
          .axes = {0},
          .shape = {1},
          .min_size = Type::kBF16,
      });
  Add(suffix,
      {
          ...
          .source_names = {"attn/kv_einsum/ClippedEinsum_0/clip_output_min",
                           "attn/k_einsum/ClippedEinsum_0/clip_output_min"},
          .axes = {0},
          .shape = {1},
          .min_size = Type::kBF16,
      });
  Add(suffix,
      {
          ...
          .source_names = {"attn/kv_einsum/ClippedEinsum_0/clip_output_max",
                           "attn/k_einsum/ClippedEinsum_0/clip_output_max"},
          .axes = {0},
          .shape = {1},
          .min_size = Type::kBF16,
      });
  ...
  Add(suffix,
      {
          .base_name = "qkv2_w",
          .source_names = {layer_config.kv_heads == 1 ? "attn/k_einsum/w"
                                                      : "attn/kv_einsum/w"},
  ...
  if (layer_config.IsMoE()) {
    ...
    const size_t expert_hidden_dim = layer_config.ff_hidden_dim / 3;
    for (uint32_t i = 0; i < layer_config.NumExperts(); ++i) {
      const std::string moe_suffix = MoESuffix(layer_idx, i);
      Add(moe_suffix, {
                          .base_name = "gating1_w",
                          .shape = {expert_hidden_dim, config.model_dim},
                      });
      Add(moe_suffix, {
                          .base_name = "gating2_w",
                          .shape = {expert_hidden_dim, config.model_dim},
      ...
      Add(moe_suffix, {
                          .base_name = "linear_w",
                          .shape = {config.model_dim, expert_hidden_dim},
                      });
    }
  }
}
