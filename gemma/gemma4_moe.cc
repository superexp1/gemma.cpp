...
struct Gemma4MoE {
  ...
  static HWY_NOINLINE void ChooseExperts(const LayerWeightsPtrs& layer,
    ...
    for (size_t token_idx = 0; token_idx < num_tokens; ++token_idx) {
      ...
      for (size_t col = 0; col < model_dim; ++col) {
        router_in_row[col] = BF16(pre_ffw_row[col]);
      }
    }
    ...
    for (size_t token_idx = 0; token_idx < num_tokens; ++token_idx) {
      ...
      for (size_t col = 0; col < model_dim; ++col) {
        ...
        if (has_router_scale) {
          val *= hwy::ConvertScalarTo<float>(scale_ptr[col]);
        }
        router_in_row[col] = BF16(val);
      }
    }
    ...
    activations.s_router_logits.Notify(layer.layer_idx, activations.router_logits,
                                       env.ctx);

    // Lightweight tasks, use within-cluster to minimize fork-join overhead.
    ParallelFor(
        Parallelism::kWithinCluster, num_tokens, env.ctx,
        ...
        [&](size_t token_idx, size_t worker) HWY_ATTR {
          ...
          hwy::HWY_NAMESPACE::VQSortStatic(logits_expert_indices, num_experts,
                                           hwy::SortDescending());
          // For softmax below.
          ...
          // Compute expert weights for the top `experts_per_token` logits
          // and store which experts are activated for this token.
          ...
          for (size_t i = 0; i < experts_per_token; ++i) {
            ...
            const uint32_t row =
                expert_sizes[expert_idx].fetch_add(1, std::memory_order_relaxed);
  ...
  static HWY_NOINLINE void ComputeExpertOutput(
    ...
    ClampInplace(layer.gtc_in, tmp_in);
    ...
    MatStorageT<BF16> C1_narrow("C1_n", Extents2D(expert_size, expert_ff_hidden_dim), env.ctx.allocator, MatPadding::kOdd);
    for (size_t i = 0; i < expert_size; ++i) {
      memcpy(C1_narrow.Row(i), C1.Row(i), expert_ff_hidden_dim * sizeof(BF16));
    }

    ClampInplace(layer.linc_in, C1_narrow);

    CallMatMul(C1_narrow, layer.moe_linear_w[expert_idx],
               /*add=*/nullptr, env, expert_out, options);
    
    ClampInplace(layer.linc_out, expert_out);
    ...
    if (layer.p_expert_sc.HasPtr()) {
      ...
      for (size_t i = 0; i < expert_size; ++i) {
        ...
        for (size_t j = 0; j < model_dim; ++j) {
          row_data[j] =
              BF16(hwy::ConvertScalarTo<float>(row_data[j]) * expert_scale);
        }
      }
    }
  ...
  static HWY_NOINLINE void ComputeAllExpertOutputs(
    ...
    ParallelFor(outer, num_activated_experts, env.ctx,
                ...
                [&](size_t expert_idx_idx, size_t cluster_idx) {
                  const size_t expert_idx = activated_expert_idx[expert_idx_idx];
                  ComputeExpertOutput(
  ...
  // For each token in parallel, fills `activations.ffw_out` with the weighted sum.
  static HWY_NOINLINE void WeightedSumOfExperts(
    ...
    const size_t experts_per_token = layer.layer_config.NumExpertsPerDatapoint();
    const size_t model_dim = activations.attention.config.model_dim;
                ...
                [&](size_t token_idx, size_t worker) {
                  ...
                  for (size_t i = 1; i < experts_per_token; ++i) {
                    MulByConstAndAdd(per_token[i].weight, get_expert_row(i),
                                     activations.ffw_out.Row(token_idx),
                                     model_dim);
                  }
                });
  }
...
void Gemma4MoETransformerLayer(size_t num_tokens, const size_t layer_idx,
  ...
  const bool share_kv_cache = false;
  ...
  auto rms_norm_inplace = [&](const MatPtr& weights,
    auto& inout) HWY_ATTR {
    size_t w_ofs = weights.Rows() > 1 ?
      (layer_idx / (num_layers / weights.Rows())) : 0;
  ...
  if (share_kv_cache) {
    if (layer_idx >= num_layers / 2) {
      if (activations.attention.config.IsGlobalLayer(layer_idx)) {
        kv_cache_layer_idx = num_layers / 2 - 1;
      } else {
        kv_cache_layer_idx = num_layers / 2 - 2;
      }
      HWY_DASSERT(config.attention_window_sizes[kv_cache_layer_idx] ==
                  config.attention_window_sizes[layer_idx]);
    }
  }

  pre_norm(layer.pre_attention_norm_scale, activations.attention.pre_att_rms_out);
  ...
  // Dual-Path FFW
  pre_norm(layer.pre_ffw2_ns.HasPtr() ? layer.pre_ffw2_ns : layer.pre_ffw_norm_scale,
  ...
  if (layer.post_ffw_norm_scale.HasPtr()) {
    post_norm(layer.layer_config.post_norm, layer.post_ffw_norm_scale, activations.ffw_out);
  }
